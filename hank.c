...
static struct {
	...
	CollisionData mAttackCollisionData;
...
static void loadHank(void* tData) {
	...
	sprintf(path, "assets/sprites/HANK_%s_WALKING.pkg", gData.mCurrentName);
	...
	sprintf(path, "assets/sprites/HANK_%s_ATTACKING.pkg", gData.mCurrentName);
	...
	gData.mAttackCollisionData.mList = getHankAttackCollisionList();
	...
	gData.mCollisionData.mList = getHankCollisionList();
	gData.mCollision = addColliderToCollisionHandler(getHankCollisionList(), getHandledPhysicsPositionReference(gData.mPhysics), c, hasBeenHit, NULL, &gData.mCollisionData);
...
static void setAttacking() {
	...
	int colAnim = addHandledCollisionAnimation(getHankAttackCollisionList(), getHandledPhysicsPositionReference(gData.mPhysics), gData.mAttack, hasHit, NULL, &gData.mAttackCollisionData);
